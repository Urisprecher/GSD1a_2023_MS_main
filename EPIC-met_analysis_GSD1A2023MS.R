...
create_volcano_plot <- function(results_df, p_value_cutoff = 0.1, logfc_cutoff = 1.5) {
  ...
  volcano_plot <- ggplot(results_df, aes(x = logFC, y = -log10(P.Value))) +
    ...
    geom_point(data = significant_results, color = "forestgreen", alpha = 0.8) +  # significant_points
    geom_hline(yintercept = -log10(p_value_cutoff), linetype = "dashed", color = "blue") +  # log_p-value_line
    geom_vline(xintercept = c(logfc_cutoff, -logfc_cutoff), linetype = "dashed", color = "blue") +  #  logFC_lines
...
#dat = low_variance_filter(df, lvp = 0.99)
